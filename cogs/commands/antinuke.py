...
class Antinuke(commands.Cog):
  def __init__(self, bot):
    self.bot = bot
    self.bot.create_background_task(self.initialize_db(), name="Antinuke.initialize_db")
  ...
  async def antinuke(self, ctx, option: str = None):
    ...
    if option is None:
      ...
      if is_activated:
        ...
        setup_embed.description += "\n<:tick:1327829594954530896>| Checking India's role position for optimal configuration..."
        await setup_message.edit(embed=setup_embed)

        await asyncio.sleep(1)
        setup_embed.description += "\n<:tick:1327829594954530896> | Crafting and configuring the IndiaSupreme role..."
        await setup_message.edit(embed=setup_embed)
        ...
        await asyncio.sleep(1)
        setup_embed.description += "\n<:tick:1327829594954530896>| Ensuring precise placement of the IndiaSupreme role..."
        await setup_message.edit(embed=setup_embed)
        try:
        ...
        embed.set_footer(text="Successfully Enabled Antinuke for this server | Powered byIndia Devlopment™", icon_url=self.bot.user.avatar.url)
        embed.set_thumbnail(url=self.bot.user.avatar.url)
  ...
  async def on_interaction(self, interaction: discord.Interaction):
    ...
    if data.get('custom_id') == 'show_punishment':
